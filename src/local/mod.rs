...
use crate::demeter::{
	args::DemeterCommandLocal,
	style::*, utils::*
};


pub fn main(args: DemeterCommandLocal) -> i32 {
	...
	if ! folder_exists(&args.folder) {
		print_error(&format!("No such folder: {}", args.folder));
		return 1;
	}

	let mut file_count = 0;

	match harvest::harvest(&args.folder, &regexes, args.output.as_ref(), 0, args.depth, args.size) {
